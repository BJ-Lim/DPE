...
- Linear Search의 평균 검색 횟수 : (n + 1) / 2
- 색인 순차 파일
  - 순차 처리와 임의 처리 모두 가능
  - 효율적인 검색이 가능하고 삽입, 삭제, 갱신이 용이
  - 인덱스 저장 공간 및 오버플로우 처리 공간이 필요하므로 공간 효율이 안좋다
  - 접근 시간이 직접 파일보다 느림
  - 구조 : 기본영역(Prime area), 색인영역(Index area), 오버플로 영역(Overflow area)
- 정규형
  ...
  - 보이스-코드 정규형(BCNF) : 3NF + 모든 속성의 함수종속이 보키를 통해서만 만족
...
- Degree(차수) : 열의 수
- 키
  - 수퍼키(super key) : 유일성의 특성을 만족하는 속성 또는 속성들의 집합
  - 후보키(candidate key) : 수퍼키 중에서 최소성을 만족하는 것
...
- 해싱
  ...
  - Synonym : 동일한 Home Address로 인하여 충돌이 일어난 레코드들의 집합
...
- 마이크로명령어 형식
  ...
  - 종류
    - 수직적 마이크로 명령어
      - 코드화된 비트를 마이크로 명령어의 연산필드에 포함해 사용
      - 제어 기억장치의 용량을 줄임
      - 해독기를 이용해 필요 코드 수만큼 제어 신호를 확장
      - 장점) 마이크로 명령어의 비트수 감소
      - 단점) 마이크로 명령어의 코드화된 비트를 해독하기 위한 지연이 발생
    - 수평적 마이크로 명령어
      - 연산필드의 각 비트와 제어신호를 1:1로 대응시켜서, 그 수만큼 비트들로 이루어진 마이크로 명령어 사용.
      - 장점) 간단한 하드웨어, 해독 지연 시간 없음
      - 단점) 마이크로 명령어의 비트수가 길기 때문에 더 큰 용량의 제어 기억장치가 필요
  - 사이클
    - [출처](http://ehpub.co.kr/tag/%EB%A7%88%EC%9D%B4%ED%81%AC%EB%A1%9C-%EC%97%B0%EC%82%B0micro-operation/)
    - Fetch Cycle
      - 명령어를 주기억장치에서 CPU의 IR(Instruction Register)로 가져와 해석하는 단계로 제일 먼저 진행
        ```
    ...
    - Indirect Cycle
      ...
      ```
      - 1 Cycle 명령이면 Fetch Cycle로 상태 전이합니다.(F ← 0, R ← 0)
...
- 플리플롭
  - 종류
    - T : 0 입력 시 현상태 유지 / 1 입력 시 보수 기억
    - RS: 0 또는 1을 기억하거나 현상태 기억
    - D: 입력값 그대로 기억
    - M/S(마스터-슬레이브): 출력물의 일부가 입력층에 피드백되는 레이스 현상을 없애기 위해 고안된 플리플롭
- 레지스터
...
- 캐시 메모리
  ...
  - 매핑 방법
    - 직접 매핑(Direct Mapping)
      - 캐시에 저장된 데이터들이 메인 메모리에서와 동일한 배열을 가지도록 매핑
      - 적중률이 낮아질 수 있음
    - 연관 매핑(Associative Mapping)
      - 캐시에 저장된 데이터는 메인 메모리의 순서와 관련이 없다
      - 적중률이 높다
    - 세트 연관 매핑
      - 단순한 세트와 적중률을 올리기 위한 목적
      - n-way 연관 매핑이라고 함
...
- 메모리 인터리빙 
  - 대역폭을 늘려주는 효과
  - 중앙 처리 장치의 기억 모듈에 중복적인 데이터 접근을 방지하기 위해서 연속된 데이터 또는 명령어들을 기억 장치 모듈에 순차적으로 번갈아 가면서 처리하는 방식
- 메모리 버스트 : 어드레스에 액세스 할 때 미리 길이를 4개 또는 8개 정도로 지정해 놓고 연속된 데이터 읽기
- 인스트럭션 사용 빈도
  - 메모리에 접근 빈도가 많아질수록 속도가 느려지며 사용빈도는 낮아짐
- 인코더, 디코더, 멀티플렉서, 디멀티플렉서 비교
...
- 3 초과 코드(excess-3 code)
  - 감산에 유용
  - 반올림에 유용
  - 모든 비트가 동시에 0이 되는 일이 없으므로, 단선 등에 의한 신호두절을 구별할 때 이용
  - 십진수 -> 3 초과 코드
    - 십진수에 3을 더해 이진수로 만든다
...
- 2의 보수 음수값 구하기
  ...
  - ex) 8비트 -14 구하기
    1. 14 = 0000 1110
    2. 1의 보수 = 1111 0001
    3. 1 더하기 = 1111 0010 (-14의 2의 보수 표현)
- 시프트
  - 산술 시프트
    - 시프트시 부호를 고려하여 시프트(0 혹은 1 붙이기)
  - 논리 시프트
    - 시프트시 부호를 고려하지 않고 시프트(0 붙이기)
- 병렬 컴퓨터의 처리요소 성능 측정 단위
  - MIPS(Million Instructions Per Seconds) : 초당 100만개
- 채널의 종류
  - 멀티플렉서 채널(Multiplexer Channel) : '저속' 입출력 장치 여러 개를 동시에 제어하는 채널, '바이트 멀티플렉서 채널'이라고도 함
    - ex) 카드리더, 프린터
  - 셀렉터 채널(Selector Chennel) : '고속' 입출력 장치 1개와 입출력하기 위해 사용
    - ex) 자기디스크, 자기테이프, 자기 드럼
...
- 마셜링(Marshalling)
  - 하나 이상의 프로그램 또는 연속되어 있지 않은 저장 공간으로부터 데이터를 모은 다음, 데이터들을 메시지 버퍼에 넣고, 특정 수신기나 프로그래밍 인터페이스에 맞도록 그 데이터를 조직화하거나 미리 정해진 다른 형식으로 변환하는 과정을 일컫는 것
- PLA(Programmable Logic Array, 프로그램 가능한 논리 배열)
  - 프로그램이 가능한 논리 소자로, n개의 입력에 대하여 2n개 이하의 출력을 만들 수 있는 논리 
...
- 비선점 스케줄링(Non-Preemptive Scheduling)
  ...
  - 종류
    ...
    - SJF(Shortest Job First)
      - 평균 대기시간을 최소로 만드는걸 최적으로 두는 알고리즘
      - 대기 상태에 있는 프로세스의 요규시간에 대한 정확한 자료를 얻기 어렵다
    - FIFO, FCFS, 기한부(Deadline), 비선점형 우선순위
- 선점 스케줄링(Preemptive Scheduling)
  ...
  - 종류 : SRT(Shortest Remaining Time), 선점 우선순위, RR, 다단계 큐, 다단계 피드백 큐
- 페이지 교체 기법
...
- 교착상태(deadlock)
  ...
  - 발생 조건
    - 상호 배제(Mutual Exclusion) : 한 자원에 대해 여러 프로세스의 동시 접근 불가
    - 점유 및 대기(Hold and Wait) : 자원을 가지고 있는 상태에서 다른 프로세스가 사용하고 있는 자원의 반납을 기다리는 것
    - 비선점(Nonpreemption) : 다른 프로세스의 자원을 강제로 가져올 수 없음
    - 환영대기(Circular Wait) : 각 프로세스가 순환적으로 다음 프로세스가 요구하는 자원을 가지고 있는 것
  - 해결 방법
...
- 운영체제의 성능 평가 요소
  - 처리 능력 향상 (Throughput)
  - 응답시간 단축 (Turn around time)
  - 사용 가능도 증대 (Availability)
  - 신뢰도 향상 (Reliability)
- 자원 보호 기법의 종류접근
...
- 단편화
  - 내부 단편화 : 메모리 영역에 특정 작업이 들어갈 수 있을 때 낭비되는 공간
    - 내부 단편화 영역 = 메모리 블럭 크기 - 특정 작업 크기
  - 외부 단편화 : 메모리 영역에 특정 작업이 들어갈 수 없을 때의 메모리 블럭 크기
    - 외부 단편화 영역 = 해당 메모리 블럭 크기
- 분산 운영체제
  - 마이그레이션
    - data migration
...
- 객체 지향 분석 방법론
  ...
  - 럼바우(Rumbaugh) 분석 기법
    ...
    - 기능 모델링(Functional) : 자료흐름도()이용하여 처리과정을 표현
- 나선형 모델
...
- 폭포수 모형
  - 순차적인 소프트웨어 개발 프로세스(소프트웨어를 만들기 위한 프로세스)
  - 개발의 흐름이 마치 폭포수처럼 지속적으로 아래로 향함
  - 소프트웨어 요구사항 분석 단계-소프트웨어 설계-소프트웨어 구현-소프트웨어 시험-소프트웨어 통합 단계-소프트웨어 유지보수 단계
...
- Alien Code : 아주 오래되거나 참고 문서 또는 개발자가 없어 유지보수 작업이 어려운 
